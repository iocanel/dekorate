...
public class CustomResourceAnnotationProcessor extends AbstractAnnotationProcessor implements CustomResourceGenerator {
  ...
  public void add(Element element) {
    ...
    if (element instanceof TypeElement) {
      ...
      JSONSchemaProps schema = JsonSchema.from(definition);

      on(customResource != null
        ? new AnnotationConfiguration<CustomResourceConfig>(CustomResourceConfigAdapter.newBuilder(customResource).addToAttributes(Keys.JSON_SCHEMA, schema).accept(new AddClassNameConfigurator(className)))
        : new AnnotationConfiguration<CustomResourceConfig>(new CustomResourceConfigBuilder().addToAttributes(Keys.JSON_SCHEMA, schema).accept(new AddClassNameConfigurator(className))));
    }
  }
}
